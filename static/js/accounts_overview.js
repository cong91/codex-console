...
function renderDistribution(containerId, data, labelFormatter) {
	...
	if (!entries.length) {
		container.innerHTML =
			'<div class="empty-state" style="padding: 16px 0;">Chưa có dữ liệu</div>';
		return;
	}
...
function formatQuotaResetText(quota) {
	...
	if (resetAt) {
		...
		if (percent !== null && percent <= 0) {
			return `Đã dùng hết, lần tiếp theo ${absolute}`;
		}
...
function renderQuotaItem(title, quota) {
	...
	return `
        <div class="quota-item">
            ...
            <div class="quota-reset">Làm mới: ${escapeHtml(resetText)}</div>
        </div>
    `;
}
...
function renderRecentAccounts(accounts) {
	...
	if (!accounts || !accounts.length) {
		tbody.innerHTML = `
            <tr>
                <td colspan="8"><div class="empty-state">Chưa có dữ liệu tài khoản</div></td>
            </tr>
        `;
		return;
	}
...
async function loadLegacyOverview() {
	try {
		...
		renderDistribution("dist-status", data.by_status, (status) =>
			getStatusText("account", status),
		);
		renderDistribution("dist-service", data.by_email_service, (service) =>
		...
		toast.error(
			`Tải thống kê tổng quan thất bại: ${error.message || "Lỗi không xác định"}`,
		);
	}
}
...
function updatePlanFilterOptions() {
	...
	const freeCount = sourceCards.filter(
		(item) => normalizePlan(item.plan_type) === "free",
	).length;
	const plusCount = sourceCards.filter(
		(item) => normalizePlan(item.plan_type) === "plus",
	).length;
	const teamCount = sourceCards.filter(
		(item) => normalizePlan(item.plan_type) === "team",
	).length;
	const currentCount = sourceCards.filter((item) =>
	...
	select.innerHTML = `
        ...
        <option value="current">Tài khoản hiện tại (${currentCount})</option>
    `;
	select.value = ["all", "free", "plus", "team", "current"].includes(
		currentValue,
	)
		? currentValue
		: "all";
...
function updateSelectionInfo() {
	...
	info.textContent = `Đã chọn ${selected} tài khoản / Danh sách hiện tại ${total} tài khoản`;
}
...
function sortCards(cards) {
	...
	rows.sort((a, b) => {
		...
		if (sortMode === "hourly_desc") {
			return (
				(parsePercent(b.hourly_quota) ?? -1) -
				(parsePercent(a.hourly_quota) ?? -1)
			);
		}
		if (sortMode === "weekly_desc") {
			return (
				(parsePercent(b.weekly_quota) ?? -1) -
				(parsePercent(a.weekly_quota) ?? -1)
			);
		}
...
function applyCardFilters() {
	const keyword = (document.getElementById("card-search-input")?.value || "")
		.trim()
		.toLowerCase();
...
function setViewMode(mode) {
	...
	if (listBtn)
		listBtn.classList.toggle("active", overviewState.viewMode === "list");
	if (gridBtn)
		gridBtn.classList.toggle("active", overviewState.viewMode === "grid");
	if (container)
		container.classList.toggle("view-list", overviewState.viewMode === "list");
}
...
function renderPlanCards() {
	...
	container.innerHTML = overviewState.filteredCards
		.map((account) => {
			const accountId = Number(account.id);
			const checked = overviewState.selectedCardIds.has(accountId)
				? "checked"
				: "";
			...
			const hasCodeReviewQuota = Boolean(
				codeReviewQuota &&
					(codeReviewQuota.status === "ok" ||
						(codeReviewQuota.percentage !== null &&
							codeReviewQuota.percentage !== undefined) ||
						codeReviewQuota.reset_at ||
						(codeReviewQuota.reset_in_text &&
							codeReviewQuota.reset_in_text !== "-")),
			);
            ...
            <article class="quota-card ${account.current ? "is-current" : ""}">
                <div class="quota-card-head">
                    ...
                    <div class="quota-badges">
                        ${account.current ? '<span class="status-pill current">Hiện tại</span>' : ""}
                        <span class="plan-badge ${getPlanClass(account.plan_type)}">${getPlanText(account.plan_type)}</span>
                    </div>
                </div>

                ${renderQuotaItem("Hạn mức 5 giờ", account.hourly_quota)}
                ${renderQuotaItem("Hạn mức tuần", account.weekly_quota)}
                ${codeReviewHtml}
                ...
                <div class="quota-card-foot">
                    ...
                    <div class="card-actions">
                        <button class="card-action-btn" data-action="refresh" data-id="${accountId}" title="Làm mới hạn mức">⟳</button>
                        <button class="card-action-btn" data-action="export" data-id="${accountId}" title="Xuất cấu hình thẻ này">⇪</button>
                        <button class="card-action-btn danger" data-action="remove" data-id="${accountId}" title="Xóa khỏi danh sách thẻ">🗑</button>
                    </div>
                </div>
            </article>
        `;
...
async function loadPlanCards(forceRefresh = false) {
	try {
		...
		updateSelectionInfo();
		toast.error(
			`Tải thẻ gói thuê bao thất bại: ${error?.message || "Lỗi không xác định"}`,
		);
	}
}
...
async function refreshSelectedOrAll(force = true, silent = false) {
	...
	try {
		if (!targetIds.length) {
			...
			if (!silent) toast.info("Hiện không có thẻ nào có thể làm mới");
			return;
		}
		...
		await loadPlanCards(false);
		if (!silent)
			toast.success(`Đã làm mới hạn mức cho ${targetIds.length} tài khoản`);
	} catch (error) {
		if (!silent)
			toast.error(`Làm mới thất bại: ${error.message || "Lỗi không xác định"}`);
	}
}
...
async function refreshSingleCard(accountId, button = null) {
	...
	toast.info(`Đang làm mới hạn mức cho tài khoản #${accountId}...`, 1500);
	try {
		...
		if (currentDetail && currentDetail.success === false) {
			toast.warning(
				`Đã làm mới nhưng chưa lấy được hạn mức mới: ${currentDetail.error || "Không rõ nguyên nhân"}`,
				4500,
			);
		} else {
			toast.success(`Đã làm mới hạn mức cho tài khoản #${accountId}`);
		}
		await loadPlanCards(false);
	} catch (error) {
		toast.error(`Làm mới thất bại: ${error?.message || "Lỗi không xác định"}`);
...
async function removeSingleCard(accountId) {
	...
	const ok = await confirm(
		"Xác nhận xóa tài khoản này khỏi danh sách thẻ? (không xóa dữ liệu trong quản lý tài khoản)",
		"Xóa thẻ",
	);
	if (!ok) return;
	...
	toast.success("Đã xóa thẻ, bạn có thể thêm lại trong mục “Thêm tài khoản”");
}

async function loadAddableAccounts() {
	try {
		const data = await api.get("/accounts/overview/cards/selectable");
		overviewState.addableCards = Array.isArray(data?.accounts)
			? data.accounts
			: [];
	} catch (error) {
...
function renderAddableAccounts() {
	...
	const options = overviewState.addableCards.map((item) => {
		...
		const tokenTag = item.has_access_token ? "Có Token" : "Không có Token";
		return `<option value="${Number(item.id)}">${escapeHtml(item.email || "-")} (${plan}/${tokenTag})</option>`;
	});
	select.innerHTML = `<option value="">Vui lòng chọn tài khoản</option>${options.join("")}`;
}
...
function getSelectedExistingAccount() {
	...
	return (
		overviewState.addableCards.find((item) => Number(item.id) === id) || null
	);
}
...
function fillAddFormFromExistingSelection() {
	...
	setFieldValue(
		"overview-add-email-service",
		selected.email_service || "manual",
	);
	setFieldValue(
		"overview-add-subscription",
		normalizePlan(selected.subscription_type || "free"),
	);
...
async function restoreSelectedAddableAccount() {
	...
	if (!Number.isFinite(id) || id <= 0) {
		toast.warning("Vui lòng chọn một tài khoản trước");
		return;
	}
	...
	toast.success("Đã thêm tài khoản vào danh sách thẻ");
}
...
function getSelectedIds() {
	return Array.from(overviewState.selectedCardIds).filter((id) =>
...
async function submitAddAccount() {
	...
	if (selectedExisting) {
		await api.post(
			`/accounts/overview/cards/${Number(selectedExisting.id)}/attach`,
			{},
		);
		...
		await loadPlanCards(false);
		await loadLegacyOverview();
		toast.success("Đã thêm trực tiếp từ quản lý tài khoản vào danh sách thẻ");
		return;
	}

	const email = (
		document.getElementById("overview-add-email")?.value || ""
	).trim();
	const password = (
		document.getElementById("overview-add-password")?.value || ""
	).trim();
	const emailService =
		(
			document.getElementById("overview-add-email-service")?.value || "manual"
		).trim() || "manual";
	const subscriptionType = (
		document.getElementById("overview-add-subscription")?.value || ""
	).trim();
	const clientId = (
		document.getElementById("overview-add-client-id")?.value || ""
	).trim();
	const accountId = (
		document.getElementById("overview-add-account-id")?.value || ""
	).trim();
	const workspaceId = (
		document.getElementById("overview-add-workspace-id")?.value || ""
	).trim();
	const status =
		(
			document.getElementById("overview-add-status")?.value || "active"
		).trim() || "active";

	if (!email || !password) {
		toast.warning("Email và mật khẩu là bắt buộc");
		return;
	}
...
async function submitImportAccounts() {
	const input = (
		document.getElementById("overview-import-json")?.value || ""
	).trim();
	const overwrite = Boolean(
		document.getElementById("overview-import-overwrite")?.checked,
	);
	if (!input) {
		toast.warning("Vui lòng dán JSON trước");
		return;
	}
	...
	try {
		...
		toast.error(`Phân tích JSON thất bại: ${error.message || "Sai định dạng"}`);
		return;
	}
	const accounts = Array.isArray(parsed)
		...
		: Array.isArray(parsed?.accounts)
			...
			: [];
	if (!accounts.length) {
		toast.warning("JSON phải là mảng không rỗng");
		return;
	}
	...
	toast.success(
		`Nhập xong: thêm mới ${result?.created || 0}, cập nhật ${result?.updated || 0}, bỏ qua ${result?.skipped || 0}, lỗi ${result?.failed || 0}`,
	);
	await loadPlanCards(false);
	await loadLegacyOverview();
	await loadAddableAccounts();
...
async function removeSelectedAccounts() {
	...
	if (!ids.length) {
		ids = overviewState.filteredCards
			...
			.filter((id) => Number.isFinite(id));
		if (!ids.length) {
			toast.warning("Hiện không có tài khoản nào để xóa");
			return;
		}
	}

	const ok = await confirm(
		`Xác nhận xóa ${ids.length} tài khoản khỏi danh sách thẻ? (không xóa dữ liệu trong quản lý tài khoản)`,
		"Xóa thẻ hàng loạt",
	);
	if (!ok) return;

	const result = await api.post("/accounts/overview/cards/remove", {
		ids,
		select_all: false,
	});
	const removedCount = Number(result?.removed_count || 0);
	toast.success(`Xóa xong: ${removedCount} thẻ`);
...
async function exportAllVisibleAccounts() {
	const ids = overviewState.filteredCards
		...
		.filter((id) => Number.isFinite(id));
	if (!ids.length) {
		toast.warning("Hiện không có tài khoản nào để xuất");
		return;
	}
	await downloadAccountsExportJson(
		{ ids, select_all: false },
		"overview_all_accounts",
	);
	toast.success(`Đã xuất ${ids.length} tài khoản`);
}
...
async function exportSingleCard(accountId) {
	...
	if (!Number.isFinite(id)) return;
	await downloadAccountsExportJson(
		{ ids: [id], select_all: false },
		`overview_account_${id}`,
	);
	toast.success(`Đã xuất cấu hình của tài khoản #${id}`);
}
...
function updateCardNextRefreshText() {
	...
	if (!overviewState.cardNextRefreshAt) {
		el.textContent = "Lần làm mới tiếp theo --";
		return;
	}
	const remainSec = Math.max(
		0,
		Math.floor((overviewState.cardNextRefreshAt - Date.now()) / 1000),
	);
	const min = Math.floor(remainSec / 60);
	const sec = remainSec % 60;
	el.textContent = `Lần làm mới tiếp theo ${min}:${String(sec).padStart(2, "0")}`;
}
...
function bindEvents() {
	...
	if (cardRefreshSelect) {
		...
		cardRefreshSelect.addEventListener("change", () => {
			...
			overviewState.cardRefreshIntervalMin = [5, 7, 10].includes(value)
				? value
				: 7;
	...
	if (toolbarRefreshBtn) {
		toolbarRefreshBtn.addEventListener("click", async () => {
			try {
				await refreshSelectedOrAll(true, true);
				await Promise.all([loadLegacyOverview(), loadAddableAccounts()]);
				toast.success("Đã làm mới tổng quan tài khoản");
			} catch (error) {
				toast.error(
	...
	if (importBtn) {
		importBtn.addEventListener("click", () =>
			openModalById("overview-import-modal"),
		);
	}
	...
	document
		.getElementById("overview-add-close")
		?.addEventListener("click", () => closeModalById("overview-add-modal"));
	document
		.getElementById("overview-add-cancel")
		?.addEventListener("click", () => closeModalById("overview-add-modal"));
	document
		.getElementById("overview-add-existing-refresh")
		?.addEventListener("click", async () => {
			try {
				await loadAddableAccounts();
				toast.success("Đã làm mới danh sách quản lý tài khoản");
			} catch (error) {
				toast.error(
					`Làm mới thất bại: ${error?.message || "Lỗi không xác định"}`,
				);
	...
	document
		.getElementById("overview-add-existing-select")
		?.addEventListener("change", () => {
			fillAddFormFromExistingSelection();
		});
	document
		.getElementById("overview-add-existing-submit")
		?.addEventListener("click", async () => {
			try {
				await restoreSelectedAddableAccount();
			} catch (error) {
				toast.error(
					`Khôi phục thất bại: ${error?.message || "Lỗi không xác định"}`,
				);
			}
		});
	document
		.getElementById("overview-add-submit")
		?.addEventListener("click", async () => {
			try {
	...
	document
		.getElementById("overview-import-close")
		?.addEventListener("click", () => closeModalById("overview-import-modal"));
	document
		.getElementById("overview-import-cancel")
		?.addEventListener("click", () => closeModalById("overview-import-modal"));
	document
		.getElementById("overview-import-submit")
		?.addEventListener("click", async () => {
			try {
				await submitImportAccounts();
			} catch (error) {
				toast.error(`Nhập thất bại: ${error?.message || "Lỗi không xác định"}`);
			}
		});
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	initResults.forEach((item, index) => {
		if (item.status === "rejected") {
			const target = index === 0 ? "thống kê tổng quan" : "danh sách thẻ";
			toast.warning(
				`Khởi tạo ${target} thất bại, đã chuyển sang chế độ hiển thị dự phòng`,
			);
		}
	});
	restartCardAutoRefresh();
