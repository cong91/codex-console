...
const currentFilters = { status: "", email_service: "", search: "" }; // 当前筛选条件
...
function initEventListeners() {
	...
	// 批量检测订阅
	elements.batchCheckSubBtn.addEventListener(
		"click",
	...
	document
		.getElementById("batch-upload-cpa-item")
	...
	document
		.getElementById("batch-upload-sub2api-item")
		.addEventListener("click", (e) => {
			e.preventDefault();
			uploadMenu.classList.remove("active");
	...
	document
		.getElementById("batch-upload-tm-item")
		.addEventListener("click", (e) => {
			e.preventDefault();
			uploadMenu.classList.remove("active");
			handleBatchUploadTm();
	...
	elements.selectAll.addEventListener("change", (e) => {
		const checkboxes = elements.table.querySelectorAll(
	...
	document.addEventListener("click", () => {
		...
		uploadMenu.classList.remove("active");
		document
			.querySelectorAll("#accounts-table .dropdown-menu.active")
...
async function loadStats() {
	try {
		...
		elements.activeAccounts.textContent = format.number(
			data.by_status?.active || 0,
		);
		elements.expiredAccounts.textContent = format.number(
			data.by_status?.expired || 0,
		);
		elements.failedAccounts.textContent = format.number(
...
function renderAccounts(accounts) {
            ...
            <tr>
                <td colspan="9">
                    <div class="empty-state">
                        ...
                        <div class="empty-state-description">Không tìm thấy tài khoản phù hợp với bộ lọc</div>
                    </div>
                </td>
            </tr>
        `;
	...
	elements.table.innerHTML = accounts
		.map(
			(account) => `
        <tr data-id="${account.id}">
            ...
            <td class="password-cell">
                ${
									account.password
										? `<span style="display:inline-flex;align-items:center;gap:4px;">
                        <span class="password-hidden" data-pwd="${escapeHtml(account.password)}" onclick="togglePassword(this, this.dataset.pwd)" title="Bấm để xem">${escapeHtml(account.password.substring(0, 4) + "****")}</span>
            ...
            <td>
                <div style="display:flex;gap:4px;align-items:center;white-space:nowrap;">
                    ...
                    <button class="btn btn-secondary btn-sm" onclick="checkInboxCode(${account.id})">Hộp thư đến</button>
                    <div class="dropdown" style="position:relative;">
                        <button class="btn btn-secondary btn-sm" onclick="event.stopPropagation();toggleMoreMenu(this)">Khác</button>
	...
	elements.table
		...
		.forEach((cb) => {
			cb.addEventListener("change", (e) => {
				...
				// 同步全选框状态
				const allChecked = elements.table.querySelectorAll(
					'input[type="checkbox"][data-id]',
				);
				const checkedCount = elements.table.querySelectorAll(
					'input[type="checkbox"][data-id]:checked',
				).length;
				elements.selectAll.checked =
					allChecked.length > 0 && checkedCount === allChecked.length;
				elements.selectAll.indeterminate =
					checkedCount > 0 && checkedCount < allChecked.length;
				updateBatchButtons();
				renderSelectAllBanner();
			});
		});
	...
	// 渲染后同步全选框状态
	const allCbs = elements.table.querySelectorAll(
		'input[type="checkbox"][data-id]',
	);
	const checkedCbs = elements.table.querySelectorAll(
		'input[type="checkbox"][data-id]:checked',
	);
	elements.selectAll.checked =
		allCbs.length > 0 && checkedCbs.length === allCbs.length;
	elements.selectAll.indeterminate =
...
function renderAccountStatusDot(status) {
	const normalized = String(status || "")
		.trim()
		.toLowerCase();
	const dotClass = ["active", "expired", "banned", "failed"].includes(
		normalized,
	)
		? normalized
		: "unknown";
...
function renderSubscriptionStatus(subscriptionType) {
	...
	const label = subscribed ? normalized.toUpperCase() : "MIỄN PHÍ";
	const title = subscribed
		? `Đã đăng ký gói: ${normalized}`
		: "Chưa phát hiện gói Plus/Team";
	return `
...
function updatePagination() {
	...
	elements.pageInfo.textContent = `Trang ${currentPage} / Tổng ${totalPages} trang`;
}
...
function renderSelectAllBanner() {
	...
	const totalPages = Math.ceil(totalAccounts / pageSize);
	const currentPageSize = elements.table.querySelectorAll(
		'input[type="checkbox"][data-id]',
	).length;
	const checkedOnPage = elements.table.querySelectorAll(
		'input[type="checkbox"][data-id]:checked',
	).length;
	const allPageSelected =
		currentPageSize > 0 && checkedOnPage === currentPageSize;
	...
	if (!banner) {
		...
		banner.style.cssText =
			"background:var(--primary-light,#e8f0fe);color:var(--primary-color,#1a73e8);padding:8px 16px;text-align:center;font-size:0.875rem;border-bottom:1px solid var(--border-color);";
	...
	if (selectAllPages) {
		banner.innerHTML = `Đã chọn toàn bộ <strong>${totalAccounts}</strong> bản ghi.<button onclick="resetSelectAllPages()" style="margin-left:8px;color:var(--primary-color,#1a73e8);background:none;border:none;cursor:pointer;text-decoration:underline;">Bỏ chọn tất cả</button>`;
	} else {
		banner.innerHTML = `Trang hiện tại đã chọn hết <strong>${checkedOnPage}</strong> mục.<button onclick="selectAllPagesAction()" style="margin-left:8px;color:var(--primary-color,#1a73e8);background:none;border:none;cursor:pointer;text-decoration:underline;">Chọn toàn bộ ${totalAccounts} mục</button>`;
	}
}
...
function updateBatchButtons() {
	...
	elements.batchDeleteBtn.textContent =
		count > 0 ? `🗑️ Xóa (${count})` : "🗑️ Xóa hàng loạt";
	elements.batchRefreshBtn.textContent =
		count > 0 ? `🔄 Làm mới (${count})` : "🔄 Làm mới Token";
	elements.batchValidateBtn.textContent =
		count > 0 ? `✅ Xác minh (${count})` : "✅ Xác minh Token";
	elements.batchUploadBtn.textContent =
		count > 0 ? `☁️ Tải lên (${count})` : "☁️ Tải lên";
	elements.batchCheckSubBtn.textContent =
		count > 0 ? `🔍 Kiểm tra (${count})` : "🔍 Kiểm tra gói";
}
...
async function refreshToken(id) {
	try {
		...
		if (result.success) {
			...
			toast.error(
				"Làm mới thất bại: " + (result.error || "Lỗi không xác định"),
			);
		}
	} catch (error) {
...
async function handleBatchRefresh() {
	...
	const confirmed = await confirm(
		`Xác nhận làm mới Token cho ${count} tài khoản đã chọn?`,
	);
	if (!confirmed) return;
	...
	try {
		const result = await api.post(
			"/accounts/batch-refresh",
			buildBatchPayload(),
		);
		toast.success(
			`Đã làm mới ${result.success_count} mục, thất bại ${result.failed_count} mục`,
		);
		loadAccounts();
	} catch (error) {
...
async function handleBatchValidate() {
	...
	try {
		const result = await api.post(
			"/accounts/batch-validate",
			buildBatchPayload(),
		);
		toast.info(
			`Hợp lệ: ${result.valid_count}, không hợp lệ: ${result.invalid_count}`,
		);
		loadAccounts();
	} catch (error) {
	...
	try {
		...
		elements.modalBody.innerHTML = `
            <div class="info-grid">
                ...
                <div class="info-item">
                    <span class="label">Lần làm mới cuối</span>
                ...
                <div class="info-item" style="grid-column: span 2;">
					<span class="label">Mã truy cập</span>
                ...
                <div class="info-item" style="grid-column: span 2;">
					<span class="label">Mã làm mới</span>
                    <div class="value" style="font-size: 0.7rem; word-break: break-all; font-family: var(--font-mono); background: var(--surface-hover); padding: 8px; border-radius: 4px;">
                ...
                <div class="info-item" style="grid-column: span 2;">
                    ...
                    <div class="value" style="font-size: 0.7rem; word-break: break-all; font-family: var(--font-mono); background: var(--surface-hover); padding: 8px; border-radius: 4px;">
						...
						<button class="btn btn-ghost btn-sm" onclick="editSessionToken(${id}, '${escapeHtml(tokens.session_token || account.session_token || "")}')" style="margin-left: 8px;" title="Chỉnh sửa mã phiên">✏️</button>
                        ${tokens.session_token_source ? `<span style="margin-left:8px;color:var(--text-muted);font-size:0.72rem;">Nguồn: ${escapeHtml(tokens.session_token_source)}</span>` : ""}
                    </div>
                </div>
                <div class="info-item" style="grid-column: span 2;">
                ...
                <div class="info-item" style="grid-column: span 2;">
					<span class="label">Cookie thanh toán</span>
                    <div class="value">
                        <textarea id="cookies-input-${id}" rows="3"
                            ...
                            placeholder="Dán chuỗi cookie đầy đủ, để trống để xóa">${escapeHtml(account.cookies || "")}</textarea>
...
async function bootstrapSessionToken(id) {
	try {
		const result = await api.post(
			`/payment/accounts/${id}/session-bootstrap`,
			{},
		);
		if (result && result.success) {
			toast.success("Bổ sung mã phiên thành công");
		} else {
			toast.warning(result?.message || "Chưa bổ sung được mã phiên");
		}
	} catch (error) {
		toast.error("Bổ sung mã phiên thất bại: " + error.message);
	} finally {
...
async function editSessionToken(id, currentToken = "") {
	...
	const nextToken = window.prompt(
		"Nhập mã phiên mới (để trống để xóa)",
		current,
	);
	if (nextToken === null) return;
	try {
		await api.patch(`/accounts/${id}`, {
			session_token: String(nextToken).trim(),
		});
		toast.success("Đã cập nhật mã phiên");
	} catch (error) {
		toast.error("Cập nhật mã phiên thất bại: " + error.message);
	} finally {
		await viewAccount(id);
		loadAccounts();
	}
...
async function deleteAccount(id, email) {
	const confirmed = await confirm(
		`Xác nhận xóa tài khoản ${email}? Thao tác này không thể hoàn tác.`,
	);
	if (!confirmed) return;
...
async function handleBatchDelete() {
	...
	const confirmed = await confirm(
		`Xác nhận xóa ${count} tài khoản đã chọn? Thao tác này không thể hoàn tác.`,
	);
	if (!confirmed) return;
	...
	try {
		...
		toast.success(`Đã xóa thành công ${result.deleted_count} tài khoản`);
		selectedAccounts.clear();
		selectAllPages = false;
		loadStats();
		loadAccounts();
...
async function exportAccounts(format) {
	...
	try {
		...
		if (!response.ok) {
			throw new Error(`Xuất thất bại: HTTP ${response.status}`);
		}
...
function selectCpaService() {
	return new Promise(async (resolve) => {
		...
		if (services.length === 0) {
			listEl.innerHTML =
				'<div style="text-align:center;color:var(--text-muted);padding:12px;">Chưa có dịch vụ CPA nào đang bật, sẽ dùng cấu hình toàn cục</div>';
		} else {
...
async function uploadAccount(id) {
	...
	const choice = await new Promise((resolve) => {
		...
		modal.querySelector("#_upload-close").addEventListener("click", () => {
			modal.remove();
			resolve(null);
		});
...
async function uploadToCpa(id) {
	...
	try {
		...
		if (result.success) {
			...
			toast.error(
				"Tải lên thất bại: " + (result.error || "Lỗi không xác định"),
			);
		}
	} catch (error) {
...
async function handleBatchUploadCpa() {
	...
	const confirmed = await confirm(
		`Xác nhận tải ${count} tài khoản đã chọn lên CPA?`,
	);
	if (!confirmed) return;
	...
	try {
		...
		if (result.failed_count > 0)
			message += `, thất bại: ${result.failed_count}`;
		if (result.skipped_count > 0)
			message += `, bỏ qua: ${result.skipped_count}`;
...
async function handleBatchCheckSubscription() {
	...
	if (count === 0) return;
	const confirmed = await confirm(
		`Xác nhận kiểm tra trạng thái gói của ${count} tài khoản đã chọn?`,
	);
	if (!confirmed) return;
...
function selectSub2ApiService() {
	return new Promise(async (resolve) => {
		...
		if (services.length === 0) {
			listEl.innerHTML =
				'<div style="text-align:center;color:var(--text-muted);padding:12px;">Chưa có dịch vụ Sub2API nào đang bật, sẽ tự chọn dịch vụ đầu tiên</div>';
		} else {
			listEl.innerHTML = services
				.map(
					(s) => `
...
async function handleBatchUploadSub2Api() {
	...
	const confirmed = await confirm(
		`Xác nhận tải ${count} tài khoản đã chọn lên Sub2API?`,
	);
	if (!confirmed) return;
...
function selectTmService() {
	return new Promise(async (resolve) => {
		...
		if (services.length === 0) {
			listEl.innerHTML =
				'<div style="text-align:center;color:var(--text-muted);padding:12px;">Chưa có dịch vụ Team Manager nào đang bật, sẽ tự chọn dịch vụ đầu tiên</div>';
		} else {
			listEl.innerHTML = services
				.map(
					(s) => `
...
async function uploadToTm(id) {
	...
	try {
		...
		if (result.success) {
			...
			toast.error(
				"Tải lên thất bại: " + (result.message || "Lỗi không xác định"),
			);
		}
	} catch (e) {
		toast.error("Tải lên thất bại: " + e.message);
...
async function handleBatchUploadTm() {
	...
	const confirmed = await confirm(
		`Xác nhận tải ${count} tài khoản đã chọn lên Team Manager?`,
	);
	if (!confirmed) return;
...
async function checkInboxCode(id) {
	...
	try {
		...
		if (result.success) {
			...
			toast.error(
				"Tra cứu thất bại: " + (result.error || "Chưa nhận được mã xác minh"),
			);
		}
	} catch (error) {
		toast.error("Tra cứu thất bại: " + error.message);
	}
}
	...
	elements.modalBody.innerHTML = `
        <div style="text-align:center; padding:24px 16px;">
            <div style="font-size:13px;color:var(--text-muted);margin-bottom:12px;">
                Mã xác minh mới nhất của ${escapeHtml(email)}
            </div>
